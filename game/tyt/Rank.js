...
var Dpr = 1.0;//window.devicePixelRatio > 2 ? 2 : window.devicePixelRatio; // 当前屏幕的Dpr， i7p 设置3 会挂
...
Rank.prototype.showFriendRankList = function (opt) {
    this.hide2D();
    this.showState = true;
    opt = opt || {};
    this.canvasType = CANVASTYPE['friendRank']; // 当前绘制的2D场景
    this.myUserInfo = MStorage.getMyUserInfo() || {}; // 更新用户信息
    this.myUserInfo.week_best_score = opt.week_best_score || 0;
    this._createPlane();
    this._updateClip();
    this._drawRankListBg(); // 背景绘制
    this.renderRankList(MStorage.getFriendsScore());
}
Rank.prototype.showGroupRankList = function (list, myUserInfo) {
    this.hide2D();
    this.showState = true;
...
Rank.prototype.showGameOverPage = function (opt) {
    this.hide2D();
...
game_cnt : 5
    ...
    // 找出那个排行 -
    this.myUserInfo = MStorage.getMyUserInfo() || { headimg: '', nickname: '', week_best_score: 0, score_info: [{ score: 0 }] // 更新用户信息
    ...
    var friendRankList = MStorage.getFriendsScore() || [];
    ...
    if (opt.score >= opt.highest_score || opt.score >= this.myUserInfo.last_week_best_score) {
        ...
        var userInfo = MStorage.getMyUserInfo() || { headimg: '', nickname: '', week_best_score: 0, score_info: [{ score: 0 }] };
        userInfo.week_best_score = opt.score;
        var friendRank1 = MStorage.getFriendsScore() || [];
        this.changlleList = [];
...
Rank.prototype.showStartPage = function (opt) {
    this.hide2D();
    if (DEBUG) return;
    this.showState = true;
...
Rank.prototype.showPkPage = function (opt) {
    this.hide2D();
    ...
    this.myUserInfo = this.data.pkListInfo[this.myidx - 1] || MStorage.getMyUserInfo();
    this.renderRankList(this.data.pkListInfo);
    this._drawPKListBg();
}
Rank.prototype.showLookersPage = function (opt) {
    this.hide2D();
    this.showState = true;
...
Rank.prototype.showBeginnerPage = function () {
    this.hide2D();
    this.showState = true;
...
Rank.prototype.hide2D = function () {
    if (DEBUG) return;
...
Rank.prototype.hide2DGradually = function () {
    if (DEBUG) return;
    var that = this;
    for (var i = 0; i < planList.length; i++) {
        if (!this.obj[planList[i]]) continue;
        TweenAnimation.customAnimation.to(this.material[planList[i]], 1, { opacity: 0, onComplete: function (i) {
            return function () {
...
// ----------------- 滑动事件处理 -----------------
Rank.prototype._findDelta = function (e) {
...
Rank.prototype.doTouchStartEvent = function (e) {
...
Rank.prototype.doTouchMoveEvent = function (e) {
    if (!this.showState) return;
    if (this.canvasType == CANVASTYPE['friendRank'] || this.canvasType == CANVASTYPE['groupRank'] || this.canvasType == CANVASTYPE['pk']) {
        var touchInfo = this._touchInfo;
...
Rank.prototype.doTouchEndEvent = function (e) {
    if (!this.showState) return;
    // 触摸返回按钮
    var pageX = e.changedTouches[0].pageX;
    var pageY = e.changedTouches[0].pageY;
    ...
    if (isClick) {
        ...
        if (this.canvasType == CANVASTYPE['start']) {
            ...
            if (pageX > 100 && pageX < 320 && pageY > 515 && pageY < 645) {
                ...
                console.log('----_drawStartClickRevert------', pageX, pageY, this.options)
...
Rank.prototype.updatePosition = function (scrollY) {
    var viewS; // 好友/ 群排行
    if (scrollY > 0) {
        // 表头下拉效果
        scrollY = 0;
...
// ----------------- 列表绘制 -----------------
Rank.prototype._drawList = function (offset, type) {
    var _this = this;
...
Rank.prototype.renderRankList = function (res) {
    var rank_list = [];
...
Rank.prototype._drawGameOverBtnClick = function () {
// 再玩一局的点击态
...
Rank.prototype._drawGameOverBtnClickRevert = function () {
// 再玩一局的点击态
    var ctx = this.context['btn'];
    ctx.clearRect(this._cx(91), this._cy(567), this._cwh(232), this._cwh(94));
    this._drawImageCenter('res/replay.png', this._cx(207), this._cy(607), this._cwh(212), this._cwh(84), 'btn', null, this.imgid['btn']);
}
Rank.prototype._drawStartClick = function () {
// 再玩一局的点击态
    console.log('---_drawStartClick----')
    var ctx = this.context['btn'];
...
Rank.prototype._drawStartClickRevert = function () {
// 再玩一局的点击态
    console.log('---_drawStartClickRevert----')
    var ctx = this.context['btn'];
    ctx.clearRect(this._cx(91), this._cy(547), this._cwh(232), this._cwh(94));
    this._drawImageCenter('res/play.png', this._cx(207), this._cy(587), this._cwh(212), this._cwh(84), 'btn', null, this.imgid['btn']);
}
Rank.prototype._drawPKListBg = function () {
// 绘制背景图
...
Rank.prototype._drawRankListBg = function () {
// 绘制背景图
...
Rank.prototype._drawGameOver = function () {
    var _this2 = this;
...
Rank.prototype._drawStart = function (opt) {
    opt = opt || {};
    // 绘制背景图
    var ctx = this.context['bg'];
    ctx.clearRect(0, 0, WIDTH, HEIGHT);
...
Rank.prototype._drawLookers = function (opt) {
// 绘制背景图
    var ctx = this.context['bg'];
    ctx.clearRect(0, 0, WIDTH, HEIGHT);
...
Rank.prototype._drawGameOverHighest = function (opt, type) {
    this.imgid['bg']++;
    opt.score = opt.score || 0;
...
Rank.prototype._reDrawChangeAva = function (pos) {
    var _this3 = this;
...
Rank.prototype._drawBeginner = function () {
    var ctx = this.context['bg'];
    ctx.clearRect(0, 0, WIDTH, HEIGHT);
...
Rank.prototype._createPlane = function () {
// 创建画布
...
Rank.prototype._updatePlane = function (type) {
// 画布更新
    if (!this.showState) {
        return;
    }
...
Rank.prototype._updateClip = function () {
// 更新切面位置
...
Rank.prototype._cname = function (x, namelen) {
    namelen = namelen || 16;
    x = x || '';
...
Rank.prototype._sliceName = function (x, namelen) {
    x = x || '';
    var len = x.replace(/[^\x00-\xff]/g, '**').length;
    if (len > namelen) {
...
Rank.prototype._cwh = function (x) {
    var realx = x * W / 414;
    if (H / W < 736 / 414) {
        // 某4
        realx = x * H / 736;
...
Rank.prototype._cf = function (size, is_num) {
// font size
    ...
    if (!!is_num) return realf + ('px ');else return realf + 'px Helvetica';
    // if (!!is_num && !!family) return realf + ('px ' + family);else return realf + 'px Helvetica';
}
Rank.prototype._cxp = function (x) {
    return x / W * 414;
}
Rank.prototype._cyp = function (y) {
// 根据坐标反推出y
    var really;
    if (H / W > 736 / 414) {
        // 某X
...
Rank.prototype._negatePlane = function (plane, point) {
    if (!plane || !point) {
        return;
    }
...
// ----------------- 工具函数 -----------------
Rank.prototype._drawImageCenter = function (src, x, y, width, height, type, cb, imgid, noupdate) {
...
Rank.prototype._drawImageRound = function (src, x, y, width, height, type, cb, imgid, noupdate) {
    // imgid 是渲染时候的imgid， 在每次改变画布的时候自增
    // 以xy为中心来显示一副图片
    if (src == '/0' || src == '/96' || src == '/64' || !src) {
        src = 'res/ava.png';
...
Rank.prototype._rerank = function (array) {
    // 排行榜重新排序
    var i = 0,
        len = array.length,
        j,
...
Rank.prototype._findSelfIndex = function (element, index, array) {
    // 从排行列表中找出自己的排名
    return element.nickname === this.myUserInfo.nickname;
}
Rank.prototype._findPartner = function (element, index, array) {
    // 从排行列表中找出自己的排名
    return element.is_self === 1;
}
Rank.prototype._roundedRectR = function (x, y, width, height, radius, type) {
    var ctx = this.context[type];
    ctx.beginPath();
...
Rank.prototype.explode = function (x, y, idx) {
    ...
    for (var i = 0; i < this.particles[idx].length; ++i) {
        ...
        TweenAnimation.customAnimation.to(this.particles[idx][i].position, t1, {
            x: x11,
            y: y11
        });

        TweenAnimation.customAnimation.to(this.particles[idx][i].position, t2, {
            x: x1,
            y: y1,
            delay: t1
        });
...
Rank.prototype.showFinger = function () {
    var _this4 = this;

    return;
    if (!this.hand) {
        this.hand = new THREE.Mesh(new THREE.PlaneGeometry(2, 3.4), new THREE.MeshBasicMaterial({ map: Config.loader.load('res/hand.png'), transparent: true }));
    ...
    TweenAnimation.customAnimation.to(this.circle.material, 0.1, { opacity: 1 });
    TweenAnimation.customAnimation.to(this.circle.scale, 0.3, { x: 1.5, y: 1.5, z: 1.5 });
    TweenAnimation.customAnimation.to(this.circle.material, 0.1, { opacity: 0, delay: 1.3 });

    TweenAnimation.customAnimation.to(this.hand.scale, 0.5, { x: 1, y: 1, delay: 1 });
    TweenAnimation.customAnimation.to(this.hand.scale, 0.5, { x: 1.5, y: 1.5, delay: 2 });
...
Rank.prototype.clearFinger = function () {
    if (this.fingerTimer) {
