...
THREE.Text = function (text, options) {
    ...
    this.obj.name = 'text';
    if (options.chinese) {
        var chinese = new THREE.Mesh(new THREE.TextGeometry(text, { 'font': THREE.myFONT, 'size': 1.0, 'height': 0.1 }), this.material);
        ...
        this.scores = [];
        this.plus = new THREE.Mesh(new THREE.TextGeometry('+', { 'font': THREE.myFONT, 'size': 3.0, 'height': 0.1 }), this.material);
        var amount = this.options.sumScore ? 5 : 2;
        for (var i = 0; i < 10; ++i) {
            var duplicateArr = [];
            var geometry = new THREE.TextGeometry(i, { 'font': THREE.myFONT, 'size': 3.0, 'height': 0.1 });
...
THREE.Text.prototype.setScore = function (score) {
    var perWidth = 2.5;
...
THREE.Text.prototype.changeStyle = function (obj) {
